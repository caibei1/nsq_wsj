...
//
// resizePool 调整协程数量 = 0.25 * all channel, 协程处理InFlightQueue和DeferredQueue
...
func (n *NSQD) queueScanLoop() {
	...
	for {
		...
		// 循环往线程池发送channel，直到有待处理任务的channel占比在QueueScanDirtyPercent以下，默认25%
	loop:
		...
		// 当工作线程占比  > QueueScanDirtyPercent, 继续工作。类似redis缓存淘汰策略
